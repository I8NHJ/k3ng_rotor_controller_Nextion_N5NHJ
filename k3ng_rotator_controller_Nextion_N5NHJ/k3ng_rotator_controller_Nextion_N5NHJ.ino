...
void service_nextion_display(){
  ...
  //N5NHJ unsigned long 0<X<4,294,967,295
  //N5NHJ unsigned int 0<x<65,535 
  const unsigned int NEXTION_VERY_FREQUENT_UPDATE_MS = 250;
  const unsigned int NEXTION_FREQUENT_UPDATE_MS = 500;
  const unsigned int NEXTION_LESS_FREQUENT_UPDATE_MS = 1000;

  // #define NEXTION_VERY_FREQUENT_UPDATE_MS 250
  // #define NEXTION_FREQUENT_UPDATE_MS 5000
  ...
  if (initialization_stage == 2){
    ...
    // Rotator Controller Arduino Code Version vRCVersion.txt="2022.02.20.01"???
    strcpy_P(workstring1,(const char*) F("vRCVersion.txt=\""));
    strcat(workstring1,CODE_VERSION);
    strcat(workstring1,"\"");
    sendNextionCommand(workstring1);
...
*/
  ...
  } //END   // Update Statuses if ((millis() - last_statuses_update) > NEXTION_VERY_FREQUENT_UPDATE_MS){
    ...
    case '?':
      ...
      if (input_buffer_index == 4){
        ...
        if ((input_buffer[2] == 'C') && (input_buffer[3] == 'V')) {  // \?CV Code Verson
          ...
          //N5NHJ Send Code Version to the NEXTION port
          #if defined(FEATURE_NEXTION_DISPLAY)
            char workstring1[32] = "";
            strcpy_P(workstring1,(const char*) F("vRCVersion.txt=\""));
            strcat(workstring1,CODE_VERSION);
            strcat(workstring1,"\"");
            sendNextionCommand(workstring1);
          #endif // N5NHJ Send Code Version to the Nextion display 
        }
